...
script in **`src/components/GradientField.astro`** — how many soft shapes drift
across the field, how large they are, how far they stretch as they move, and
the diagonal they travel along. The pointer feel is governed by four values:
`pointerEase` (how far the field lags behind the mouse — lower is calmer),
`cursorRadius` (how local the ripple is), `cursorPush` (how hard the pointer
nudges nearby colour), and `springBack` / `damping` (how slowly it settles).

The colours are the `--hero-*` properties in `tokens.css`. Changing those seven
values moves the whole hero between moods — a warm sunset wash, a cool jungle
shade — without touching the component. To remove the effect entirely, delete
